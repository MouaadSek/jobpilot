...
## Summary
- 3591 nodes · 9528 edges · 140 communities (120 shown, 20 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 571 edges (avg confidence: 0.53)
...
Cohesion: 0.17
Nodes (44): _client(), _days_ago(), _events(), _no_advisor_client(), _offer_application(), Connection, MonkeyPatch, Path (+36 more)
...
Cohesion: 0.05
Nodes (108): ApplicationGenerationError, A redacted generation failure suitable for CLI and dashboard display., ExperienceFact, FactBank, FactClaim, One atomic statement that generated content may cite., GenerationWarning, One thing the reviewer is being asked to check by eye. (+100 more)
...
Cohesion: 0.12
Nodes (24): Validate the one JSON contract shared by every advisor provider., unsupported number 27001' was rejecting real, bank-backed vocabulary., test_the_observed_failure_no_longer_fails_a_generation(), _tailor(), _experience_content(), _gemini_shaped_payload(), _offer(), MonkeyPatch (+16 more)
...
Cohesion: 0.15
Nodes (36): ignore_offer(), promote_offer(), Put a below-threshold offer into the normal review flow. Returns its id., Dismiss a skimmed offer so it stops reappearing. Returns its id.      Persisted, _client(), _days_ago(), _events(), _offer() (+28 more)
...
Cohesion: 0.06
Nodes (56): apply_cmd(), backfill_descriptions_cmd(), _csv(), daemon_cmd(), init_db_cmd(), init_profile_cmd(), invention_report_cmd(), _langs() (+48 more)
...
Cohesion: 0.08
Nodes (30): ConnectionFactory, Event, IngestResult, Ingestion orchestrator: pull normalized records from a Source into the DB.  Idem, _default_model_loader(), _default_score_pass(), _production_connection(), Any (+22 more)

### Community 23 - "france_travail.py"
Cohesion: 0.06
Nodes (50): _delay(), Call fn(); retry on transient HTTP errors with full-jitter exponential backoff., with_backoff(), ATSSource, infer_contract(), load_targets(), map_greenhouse(), map_lever() (+42 more)

### Community 24 - "test_provenance_tiers.py"
Cohesion: 0.08
Nodes (36): _in_bank(), _offer(), Three kinds of token, three different burdens of proof., Not even the widest scope can support it., The reader has to be able to search for it, or add it to the config., Task 26's handling survives as the digit-shaped corner of tier 2., Presence in the bank is necessary for tier 2, never sufficient., No fact anywhere carries these figures, so no scope can accept them. (+28 more)
...
Cohesion: 0.11
Nodes (21): date, _check_orphans(), _contains_generated_orphan(), DocumentToolchain, _french_date(), generate_application(), _persist_variant(), Connection (+13 more)

### Community 31 - "test_contacts.py"
Cohesion: 0.07
Nodes (49): ModuleType, SimpleNamespace, JobPilot: personal job application pipeline for the French IT/cybersecurity mark, _application(), _ConnectionProxy, Connection, Exception, Path (+41 more)
...
Cohesion: 0.08
Nodes (53): get_or_create_company(), CompanyRecord, _fixture(), _no_real_sleeping(), _NoWait, Connection, LogCaptureFixture, MonkeyPatch (+45 more)
...
Cohesion: 0.09
Nodes (38): clear_warnings(), _decode(), Connection, What a generation had to degrade, recorded where the reviewer will see it.  Task, Gate names per application, for the library and tracker markers.      One query, Replace this application's warnings with the ones from this run., Drop the previous run's warnings at the start of a new generation.      Cleared, Every warning the current generation of this application carries. (+30 more)
...
Cohesion: 0.05
Nodes (43): AmbiguousFactIdError, _fact_id_key(), _guessed_section(), nearest_entry_claim_ids(), CompletedProcess, Raised when generated prose states a figure the bank does not contain.      A si, Raised when a citation matches no fact id, even after normalisation.      ``sect, Fold separator and case differences, and nothing else, for comparison. (+35 more)
...
Cohesion: 0.14
Nodes (27): Decision, RouteId, Remove configured secrets from exception text before display/logging., Settings, _applicant_reason(), _ats_prefill(), _email(), has_form_mapping() (+19 more)
...
Cohesion: 0.10
Nodes (46): ingest_source(), Run one source end to end. Commits once at the end for atomicity.      Every out, _consecutive_failures(), _last_runs(), Any, Connection, Row, Leading failures only: one success resets the streak. (+38 more)
...
Cohesion: 0.16
Nodes (20): Freshness, How old one offer is, and how sure we are of that., application_detail(), event_history(), offer_freshness(), outreach_drafts(), Any, Connection (+12 more)
...
Cohesion: 0.12
Nodes (23): AnthropicTailoringAdvisor, build_advisor(), Claude Messages API adviser used when ``ANTHROPIC_API_KEY`` is set., Resolve TAILORING_PROVIDER to a concrete mode, without building anything.      C, Select the configured provider without silently bypassing missing keys., resolve_provider(), _Client, _offer() (+15 more)
...
Cohesion: 0.05
Nodes (50): FastAPI, Request, open_manually(), The manual_open route: open the offer, copy the letter, submit nothing.      A l, ApplicationNotQueuedError, archive_artifacts(), generation_single_flight(), GenerationInFlight (+42 more)
...
Cohesion: 0.14
Nodes (26): LookupError, ApplicationNotFoundError, ApplyOutcome, Shared human-approval and document-generation application flow., Raised when an approval targets an unknown application., The result shared by the CLI and dashboard approval surfaces., current_status(), IllegalTransition (+18 more)
...
Cohesion: 0.13
Nodes (22): max_offer_age_days(), The instant a row must be at or after to survive the staleness filter.      For, stale_cutoff(), available_sources(), _create_application(), Connection, datetime, Row (+14 more)
...
Cohesion: 0.19
Nodes (14): source_id(), _backfill_company_source(), _insert_offer(), Connection, INSERT OR IGNORE one offer. Returns True if a new row was created., Append one row to source_runs. Does not commit; the caller owns that.      A fai, Teach an existing company row where it came from, once.      A company first see, record_run() (+6 more)
...
Cohesion: 0.05
Nodes (79): ApplicantProfile, The non-secret contact values entered into an ATS form., build_prefill(), discard_mapping(), fields_from_html(), FormField, FormLearningError, FormMapping (+71 more)
...
Cohesion: 0.12
Nodes (22): _offer(), _OneShotProfileOrphan, Connection, Path, _Toolchain, Systemic recovery at the generated-prose and document-layout boundaries., Naming the employer is what a motivation letter does.      The rule was never lo, A profile-only layout regression that disappears with template wording. (+14 more)
...
### Community 130 - "score"
Cohesion: 0.07
Nodes (46): Logger, ingest_cmd(), Fetch offers from a source (or all sources) into the database., Put text on the system clipboard, or say plainly that it could not.  The manual_, _env_bool(), get_settings(), _path(), Configuration and path resolution. Secrets come from .env only (never mocked). (+38 more)
...
Cohesion: 0.10
Nodes (38): age_in_days(), annotate(), describe(), drop_stale(), _label(), _parse(), Any, datetime (+30 more)
